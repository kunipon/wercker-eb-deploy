...
if [ ! -n "$WERCKER_EB_DEPLOY_REGION" ]; then
  #set default region ap-northeast-1
...
# construct s3 cp dest object
CP_DST_OBJ="s3://$WERCKER_EB_DEPLOY_S3_BUCKET"
if [ -n "$WERCKER_EB_DEPLOY_S3_BUCKET_PATH" ]; then
    CP_DST_OBJ=$CP_DST_OBJ/$WERCKER_EB_DEPLOY_S3_BUCKET_PATH
fi
CP_DST_OBJ=$CP_DST_OBJ/$WERCKER_EB_DEPLOY_S3_CP_DST

aws s3 cp --acl private "$WERCKER_EB_DEPLOY_S3_CP_SRC" "$CP_DST_OBJ"
